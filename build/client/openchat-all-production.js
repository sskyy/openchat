...
(function() {
  ...
  window._OPENCHAT_BUILD = '1359871691000';
  ...
  */
  ...
  angular.module('openchat.service').service('$user', function($q, $http, $window) {
    ...
    user_login = function() {
      ...
      $http.jsonp("" + base + "/oauth/apply_oauth_id?callback=JSON_CALLBACK").success(function(data) {
        ...
        param = ['?client_id=3394267529', 'redirect_uri=openchat.angularjs.cn/oauth/callback', 'forcelogin=true', 'state=weibo:' + oauth_id].join('&');
  ...
  angular.module('openchat.directive').directive('ngScrollToBottom', function() {
    return function($scope, ele, attr) {
      return ele.bind('DOMNodeInserted', function(e) {
        if (e.relatedNode === ele[0]) {
          console.log('begin to scroll');
          return window.setTimeout(function() {
            return ele[0].scrollTop = ele[0].scrollHeight;
          }, 200);
        }
      });
    };
...
(function() {
  ...
  url = "http://openchat.angularjs.cn:80";
  ...
  angular.module('initialize', []).run(function($http, $window) {
    ...
    if ('' === 'auto') {
      return $window.setInterval(function() {
        return $http.jsonp("" + url + "/build_spy/get_build?callback=JSON_CALLBACK").success(function(res) {
          if (res.build !== void 0 && res.build !== _OPENCHAT_BUILD) {
